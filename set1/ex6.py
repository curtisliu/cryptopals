...
def normalized_hamming_distance_for_keysize(s, keysize):
  total_distance = 0
  blocks = 3
...
def find_keysize(s):
  distances = {}
  for i in range(2, 40):
    distances[i] = normalized_hamming_distance_for_keysize(s, i)
...
def decrypt_repeating_key_xor(s):
  for keysize, score in list(find_keysize(s))[:5]:
